...
<div class="row">
    ...
    <?= $form->field($model, 'level')
             ->dropDownList(ArrayHelper::map($levelList, 'level', 'value'),
                     ['prompt' => ' - Укажите уровень'])
             ->hint('Укажите уровень страхового представителя') ?>
    <?= $form->field($model, 'login')->textInput()->hint('Укажите логин пользователя') ?>

    <?= $form->field($model,'company_id')->widget(Select2::classname(), [
            'data' => ArrayHelper::map($companyModel, 'company_id', 'company_name'),
            'language' => 'ru',
            'options' => ['placeholder' => 'Укажите организацию ...'],
            'pluginOptions' => [
                'allowClear' => true
            ],
        ]);
